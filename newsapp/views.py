...
def random_article():
	...
	for i in range(10):
		...
		article_description = "This is the desription for article {}".format(chosen_category)
...
def edit_comment(request):
	if not request.user.is_authenticated:
		context = {"message": "You need to be logged in to post a comment on any article. "}
...
def create_comment(request):
	...
	context = {"comment": serializers.serialize("json", [comment,]), "user": serializers.serialize("json", [user,])}
	return HttpResponse(json.dumps(context), content_type="application/json")

def article_page(request, article_id):
	article = Article.objects.get(id=article_id)
	all_comments = Comment.objects.filter(article=article).order_by('-comment_created_time')
	context = {"article": article, "all_comments": all_comments}
...
def get_distinct_categories(request):
	...
	unique_categories = list(Article.objects.all().values_list('article_category', flat=True).distinct())
...
def get_favourite_categories(request):
	...
	favourite_categories = profile.favourite_category.split(",") if profile.favourite_category else []
...
def auth_login(request):
	if request.method == "POST" and "login_account" in request.POST:
		...
		if user:
			...
			context = {"message": "Oops! There's an issue with your credentials!"}
...
def browse_category(request, category_value):
	...
	articles = Article.objects.filter(article_category__icontains=category_value)
	print(articles)
...
def create_account(request):
	if request.method == "POST" and "create_account" in request.POST:
		...
		new_profile = Profile.objects.create(user=new_user,date_of_birth=date_of_birth)

		# Sending an email to the user.
		email_subject = "Welcome to the UKNews."
		message = "Dear {}, \n \n Welcome to the UKNews. Our service provides exclusive news to you. \n \n Regards, \n The UKNews.".format(first_name)
		email_message = EmailMessage(email_subject, message, settings.EMAIL_HOST_USER, [email_address])
		email_message.send()
...
def profile(request):
	...
	if request.method == "POST" and "delete_picture" in request.POST:
		if profile.picture:
			previous_image = os.path.join(settings.MEDIA_ROOT, profile.picture.name)
	...
	unique_categories = Article.objects.all().values_list('article_category', flat=True).distinct()
...
def update_categories(request):
	...
	Profile.objects.filter(user=request.user.pk).update(favourite_category=new_categories)
